...
int main(int argc, char *argv[])
{
	try
	{
		...
		cout << satisfiedmatches.size() << " ACC~FUNC matches are satisfied:"
			<< endl;

		// Print all satisfied matches
		for (long idx : satisfiedmatches)
			cout << "$" << idx << ' '
				<< accfuncmatches.at(idx).accname << " ~ "
...
long ReadMatchesFromString(istream &fin)
{
	...
	while (getline(fin, line))
	{
		...
		auto it = find_if(line.begin(), line.end(), [](char c) {
			return !isspace(c);
		});
		...
		if (regex_match(line, rex_matches, rex_accfunc))
		{
			...
			string accname = rex_matches[2].str(),
				funcname = rex_matches[3].str();
			if (accfuncmatches.find(matchidx) != accfuncmatches.end())
				throw runtime_error("Duplicate match index: "
					+ to_string(matchidx));
		...
		for (const auto &vp : m.second.varsmatch)
		{
			...
			if (flag1 != flag2)
				throw logic_error("SelectSatisfiedMatched: "
					"matching inconsistency for ACC~FUNC " + to_string(m.first));
			if (!flag1)
			{	// This variable pair is not matched
				issatisfied = false;
				break;
			}
		}
